...
public class PreferrenceWidget extends Dialog {
	...
	private void createContents() {
		...
		labelOnTop.setBounds(30, 280, 62, 20);
	    ...
	    // 현재 위치를 알아오는 버튼	
		Button btnCurrent = new Button(compoWidget, SWT.NONE);
		btnCurrent.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				spinX.setValues(	TickerWidget.getInstance().getTickerX(), 0, 3000, 0, 1, 100);
				spinY.setValues(	TickerWidget.getInstance().getTickerY(), 0, 3000, 0, 1, 100);
				spinWidth.setValues(	TickerWidget.getInstance().getTickerWidth(), 0, 3000, 0, 1, 100);
			}
		});
		btnCurrent.setBounds(157, 25, 77, 25);
		btnCurrent.setText(Messages.PreferrenceWidget_getCurrent);
		spinSpacing.setValues(PreInfo.getInstance().spacing, 0, 100, 0, 1, 10);
	 ...
	 // 두번째 칼럼이 시작되는 코드 부분
	    ...
	    btnAdd.addSelectionListener(new SelectionAdapter() { //피드를 추가하는 부분
	    	...
	    	public void widgetSelected(SelectionEvent e) {
	    		...
	    		//http://sirini.net/blog/?p=573
	    		try{
	    			//만일 입력란에 http://를 붙이지 않으면 twitter 아이디를 입력한 걸로 가정하여
	    			//트위터 rss 주소를 불러온다
	    			if(urlStr.indexOf("http://")==-1) 
	    				urlStr = "http://api.twitter.com/1/statuses/user_timeline.rss?screen_name="+urlStr; 
		            ...
		            NodeList channel = doc.getElementsByTagName("channel"); //xml에서 channel 부분 파싱
		            NodeList itemlist = doc.getElementsByTagName("item"); //xml에서 item 부분 파싱
