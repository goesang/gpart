...
public class TickerWidget extends Dialog {
	...
	private final int TIMER_INTERVAL = 25; 	// 티커의 에니메이션 업데이트 간격 ( 리눅스의 경우 50이 적당합니다  그 이상은 cpu 부하 )
		...
		canvas.addMouseMoveListener(new MouseMoveListener() {
			...
			public void mouseMove(MouseEvent e) {
				...
				if (origin != null) {	// 티커 이동 및 티커 조절을 위한 Point 생성
					...
					PreInfo.getInstance().save(p.x - origin.x, p.y - origin.y,getTickerWidth());
				}

				if (resize) {	// 티커 크기 조절
					shell.setSize(e.x+5,shell.getSize().y);
					PreInfo.getInstance().save(getTickerX(), getTickerY(), e.x+5);
				}
